      ...
      {isLoading ? (
        ...
        rows.map((r: any) => (
          <CompanyRow
            key={r.id}
            row={r}
            onDecide={(decision, notes) => decide.mutate({ id: r.id, decision, notes })}
          />
        ))
      )}
    </div>
...
function CompanyRow({
  ...
  row: any;
  onDecide: (decision: "approved" | "rejected" | "pending", notes: string) => void;
}) {
  const { t } = useTranslation();
    ...
    <Card>
      <CardContent className="p-5">
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            ...
            <div className="mt-1 text-xs text-muted-foreground">
              {row.contact_email ?? "—"} · {row.phone ?? "—"} · {row.country ?? "—"}
            </div>
          </div>
          <Badge
            className={
              locked ? "bg-success/15 text-success" : rejected ? "bg-error/15 text-error" : ""
            }
          >
        ...
        <div className="mt-3 grid sm:grid-cols-2 gap-3 text-sm">
          <div>
            <span className="text-muted-foreground">{t("auth.vatNumber")}:</span>{" "}
            <span className="font-mono">{row.vat_number ?? "—"}</span>
          </div>
          <div>
            <span className="text-muted-foreground">{t("auth.crNumber")}:</span>{" "}
            <span className="font-mono">{row.cr_number ?? "—"}</span>
          </div>
        </div>
        ...
        <div className="mt-3 flex gap-2 flex-wrap">
          {locked ? (
            ...
            <>
              <Button size="sm" variant="gold" onClick={() => onDecide("approved", notes)}>
                <CheckCircle2 className="h-4 w-4" /> {t("admin.approve")}
              </Button>
              <Button size="sm" variant="destructive" onClick={() => onDecide("rejected", notes)}>
              ...
              <CardContent className="p-5 flex items-center gap-4 flex-wrap">
                ...
                <div className="flex-1 min-w-[180px]">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {h.city}, {h.country} · {h.star_rating}★
                  </div>
                  {h.description && (
                  ...
                  {isApproved ? (
                    ...
                    <>
                      <Button
                        size="sm"
                        variant="gold"
                        onClick={() => setStatusFor.mutate({ id: h.id, next: "approved" })}
                      >
                        {t("admin.approve")}
                      </Button>
                      <Button
                        size="sm"
                        variant="destructive"
                        onClick={() => setStatusFor.mutate({ id: h.id, next: "suspended" })}
                      >
                        {t("admin.suspend")}
                      </Button>
                    </>
