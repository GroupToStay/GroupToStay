  ...
  const { data: hotels = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("hotels")
        .select(
          "id,slug,name,city,country,city_id,country_id,star_rating,cover_image,description,amenities,featured",
        )
        .eq("status", "approved")
      ...
      <main className="flex-1">
        ...
        <div className="container-page py-6 grid md:grid-cols-[1fr_200px_140px] gap-3 sticky top-16 bg-background z-30 border-b border-border">
          ...
          <Select value={cityId} onValueChange={setCityId}>
            <SelectTrigger>
              <SelectValue placeholder={t("hotels.filterCity")} />
            </SelectTrigger>
            <SelectContent>
          ...
          <Select value={stars} onValueChange={setStars}>
            <SelectTrigger>
              <SelectValue placeholder={t("hotels.filterStars")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="__any">{t("hotels.any")}</SelectItem>
              {[5, 4, 3, 2, 1].map((n) => (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((h) => (
                <Link
                  key={h.id}
                  to="/hotels/$id"
                  params={{ id: h.slug ?? h.id }}
                  className="group rounded-lg overflow-hidden border border-border bg-card hover:shadow-[var(--shadow-elevated)] transition"
                >
