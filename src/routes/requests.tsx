...
export const Route = createFileRoute("/requests")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Live RFQs from group organizers. Hotels: respond and message the organizer to close the deal." },
    ],
  }),
  component: Page,
});
...
function Page() {
  ...
  const cities = useMemo(() => Array.from(new Set(rfqs.map(r => r.destination_city))).sort(), [rfqs]);
  ...
  const filtered = rfqs.filter(r => {
    ...
    if (q && !`${r.title} ${r.destination_city} ${r.destination_country}`.toLowerCase().includes(q.toLowerCase())) return false;
    return true;
  });
      ...
      <main className="flex-1">
        <section className="bg-primary text-primary-foreground">
          <div className="container-page py-14">
            ...
            <p className="mt-2 text-primary-foreground/80">Live RFQs from organizers. Hotels — review and message the organizer to win the deal.</p>
          </div>
        </section>
        ...
        <div className="container-page py-6 grid md:grid-cols-[1fr_200px_200px] gap-3 sticky top-16 bg-background z-30 border-b border-border">
          <Input placeholder="Search by destination, title…" value={q} onChange={e => setQ(e.target.value)} />
                  ...
                  <Card className="h-full hover:shadow-[var(--shadow-elevated)] transition border-border">
                    <CardContent className="p-5 space-y-3">
                      ...
                      <div className="text-sm text-muted-foreground space-y-1">
                        <div className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" /> {r.destination_city}, {r.destination_country}</div>
